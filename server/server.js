...
const objectToUint8Array = (object) => new TextEncoder().encode(JSON.stringify(object));
const uint8ArrayToObject = (uint8Array) => JSON.parse(new TextDecoder().decode(uint8Array));

var integrationsManifest = {
    'niko-home-control': './integrations/niko-home-control.js'
};

const configStore = new LeofcoinStorage('config', '.peace-ui');
await configStore.init();
if (!await configStore.has('integrations'))
    await configStore.put('integrations', objectToUint8Array([{ integration: 'niko-home-control' }]));
const integrations = {};
...
await socketRequestServer({ protocol: 'peace-api', port: 6006 }, {
    config: async ({ send }) => send((await configStore.get())),
    integrations: async ({ send }) => send(uint8ArrayToObject(await configStore.get('integrations'))),
    supportedIntegrations: async ({ send }) => send(integrationsManifest),
    addIntegration: async (params, { send }) => {
        if (!params.integration)
            return send('invalid integration');
    ...
    changeIntegration: async (params, { send }) => {
        ...
        for (const integration of integrations) {
            if (integration.integration === params.integration)
                integration.config = params.config;
        }
        await configStore.put('integrations', objectToUint8Array(integrations));
        send('ok');
    },
